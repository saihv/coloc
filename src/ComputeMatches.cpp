...
int main(int argc, char **argv)
{
	...
	const string jpg_filenameL = sInputDir + "img__Quad0_0000.png";
	const string jpg_filenameM = sInputDir + "img__Quad1_0000.png";
	const string jpg_filenameR = sInputDir + "img__Quad2_0000.png";
	...
	std::vector<PointFeatures> feats;
	feats.push_back(featsL);
	feats.push_back(featsM);
	feats.push_back(featsR);
	...
	for (const auto & pair_idx : pairs)
	{
		...
		IndMatchDecorator<float> matchDeduplicator(vec_PutativeMatches, feats[pair_idx.first], feats[pair_idx.second]);
		...
		const Pair current_pair = pair_idx;
		...
		std::vector <IndMatch> putativeMatches = map_PutativesMatches[pair_idx];
		...
		else
		{
			...
			for (int ic = 0; ic < relativePose_info.vec_inliers.size(); ++ic) {
				vec_geometricMatches.push_back(putativeMatches[relativePose_info.vec_inliers[ic]]);
			}

			if (!vec_geometricMatches.empty()) {
				geometricMatches.insert({ { pair_idx.first, pair_idx.second }, std::move(vec_geometricMatches) });
			}
	...
	if (!Save(geometricMatches,
		std::string(sInputDir + "/" + "matches.f.bin")))
	{
		std::cerr
	...
	std::vector <IndMatch> putativeMatches = map_PutativesMatches[{0, 1}];
		...
		const Pose3 pose1 = tiny_scene.poses[tiny_scene.views[1]->id_pose] = relativePoses[{0, 1}].relativePose;

		Hash_Map<IndexT, double> map_ACThreshold_;

		map_ACThreshold_.insert({ 0,  relativePoses[{0, 1}].found_residual_precision });
		map_ACThreshold_.insert({ 1,  relativePoses[{0, 1}].found_residual_precision });
		...
		for (size_t i = 0; i < relativePoses[{0, 1}].vec_inliers.size(); ++i) {
			const SIOPointFeature & LL = regionsL->Features()[putativeMatches[relativePoses[{0, 1}].vec_inliers[i]].i_];
			const SIOPointFeature & RR = regionsM->Features()[putativeMatches[relativePoses[{0, 1}].vec_inliers[i]].j_];
			// Point triangulation
			Vec3 X;
			TriangulateDLT(
			...
			landmarks[i].obs[tiny_scene.views[0]->id_view] = Observation(LL.coords().cast<double>(), putativeMatches[relativePoses[{0, 1}].vec_inliers[i]].i_);
			landmarks[i].obs[tiny_scene.views[1]->id_view] = Observation(RR.coords().cast<double>(), putativeMatches[relativePoses[{0, 1}].vec_inliers[i]].j_);
			landmarks[i].X = X;
		}
		...
		const openMVG::matching::PairWiseMatches & map_Matches = geometricMatches;
		...
		std::transform(tiny_scene.GetLandmarks().begin(), tiny_scene.GetLandmarks().end(), std::inserter(reconstructed_trackId, reconstructed_trackId.begin()), stl::RetrieveKey());
		...
		tiny_scene.views[2].reset(new View("", 2, 0, 2, imageR.Width(), imageR.Height()));
		...
		(
			resection::SolverType::P3P_KNEIP_CVPR11,
