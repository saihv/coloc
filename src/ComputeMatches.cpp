...
int main(int argc, char **argv)
{
	...
	*/
	...
	shared_track_visibility_helper_->GetTracksInImages({ 0, 1 }, map_tracksCommon);
	...
	const Pose3 Pose_I = tiny_scene.poses[tiny_scene.views[0]->id_pose] = Pose3(Mat3::Identity(), Vec3::Zero());
	const Pose3 Pose_J = tiny_scene.poses[tiny_scene.views[1]->id_pose] = relativePoses[{0, 1}].relativePose;
		...
		++iterT)
	{
		...
		Observations obs;
		obs[tiny_scene.views[0]->id_view] = Observation(x1_, i);
		obs[tiny_scene.views[1]->id_view] = Observation(x2_, j);
	...
	*/
	...
	shared_track_visibility_helper_->GetTracksInImages({ 2 }, map_tracksCommonNew);
	...
	tiny_scene.poses[view_I->id_pose] = pose;
		...
		if (!tiny_scene.GetIntrinsics().empty())
		{
			...
			std::transform(tiny_scene.GetIntrinsics().begin(), tiny_scene.GetIntrinsics().end(),
				std::inserter(existing_intrinsicId, existing_intrinsicId.begin()),
				stl::RetrieveKey());
			new_intrinsic_id = (*existing_intrinsicId.rbegin()) + 1;
		}
		tiny_scene.views.at(2)->id_intrinsic = new_intrinsic_id;
		tiny_scene.intrinsics[new_intrinsic_id] = optional_intrinsic;
